...
contract GoblinToken is BEP20 {
    ...
    event EnableTransfer(address indexed operator);
    ...
    /**
     * @dev Blocks transfers before launch
     * When transferActive is not active, only accept transfers from operator(To add liquidity).
     * This will allow us to add the pool before launch
     */
    modifier blockTransfer(address sender) { 
        if (transferActive == false) { 
            require(sender == operator(), "GOB::blockTransfer: Transfers can only be done by operator."); 
        }
        _; 
    }
     ...
     */
    constructor() public BEP20("GOBLIN Token", "GOB") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override blockTransfer(sender) {
        // swap and liquify
        if (
     ...
     */
    function enableTransfer() public onlyOperator {
        transferActive = true;
